...
function ViewLocation({healthWorkerProfile}) {
    const router = useRouter();
    const pathname = usePathname();
    const [loading, setLoading] = useState(false);

    const [activeTab, setActiveTab] = useState('/health-worker/settings/profile/location/view');
    ...
    useEffect(() => {
        if (pathname.includes('set')) {
            ...
            setActiveTab('/health-worker/settings/profile/location');
        }
    }, [pathname]);

    const mutationDelete = useMutation({
        mutationKey: ["deleteLocation"],
        mutationFn: AdminUtils.deleteHealthWorkerLocation
    });

    const setLocation = async () => {
        router.push(`/health-worker/settings/profile/location/set`);
    };

    const onEdit = (objId) => {
        useLocationStore.getState().setGeoId(objId);
        router.push('/health-worker/settings/profile/location/edit');
    };
    ...
    const confirmDelete = () => {
        ...
        mutationDelete.mutate(selectedAddressId, {
            onSuccess: () => {
                ...
                queryClient.invalidateQueries({queryKey: ["HealthWorkerData"]});
                router.refresh();
                setIsDeleting(false);
            },
            onError: (error) => {
    ...
    if (healthWorkerProfile.geoLocation?.length === 0 || !healthWorkerProfile.geoLocation) {
        return (
            <Box
                sx={{
                    display: 'flex',
            ...
            >
                ...
                <Button
                    ...
                    onClick={() => {
                        ...
                        router.push('/health-worker/settings/profile/location/set'); // Navigate to route
                    }}
                    color="primary"
                    size="large"
            ...
            >
                ...
                <Stack direction='row' spacing={2} sx={{
                    ...
                    >
                        ...
                        <Tab
                            label="View-Location"
                            href="/health-worker/settings/profile/location/view"
                            value="/health-worker/settings/profile/location/view"
                            sx={{
                                color: "#FFF",
                                fontWeight: 'bold',
                                fontSize: xSmall || small || medium || large ? '0.6rem' : '0.9rem',
                        ...
                        <Tab
                            ...
                            component={Link}
                            onClick={setLocation}
                            href="/health-worker/settings/profile/location/set"
                            value="/health-worker/settings/profile/location/set"
                            sx={{
                                color: "#FFF",
                                fontWeight: 'bold',
                                fontSize: xSmall || small || medium || large ? '0.6rem' : '0.9rem',
                ...
                >
                    <Grid container spacing={4}>
                        {healthWorkerProfile.geoLocation.map((address, index) => (
