...
export default function FullHealthCheckPage({ healthWorkerProfile, id }) {
    ...
    //construct fullName with firstName middleName(optional) and lastName from the healthWorkerProfile
    const fullName = `${healthWorkerProfile.firstName} ${healthWorkerProfile.middleName || ''} ${healthWorkerProfile.lastName}`;
    ...
    useEffect(() => {
        const checkVoteStatus = async () => {
            if (condition?.id) {
                ...
                const userAlreadyVoted = existingVotes.some((vote) => vote.healthWorkerId === healthWorkerProfile._id);
                setHasVoted(userAlreadyVoted);
            }
        };
        checkVoteStatus();
    }, [condition?.id, healthWorkerProfile._id]);
    ...
    const createMessage = async (healthCheckData) => {
        try {
            ...
            const messageData = {
                ...
                receiverId: healthWorkerProfile._id,
    ...
    const createMedicalRecord = async (medicalData) => {
        try {
            ...
            const requiredFields = {
                healthWorkerId: medicalData.healthWorkerId,
    ...
    const handleVote = async () => {
        if (!healthWorkerProfile || !condition || hasVoted) return;
    ...
    const handleSubmitRequest = async () => {
        if (!healthWorkerProfile || !condition) {
            return;
        }
        ...
        try {
            ...
            const userRequestRef = doc(db, 'userRequests', healthWorkerProfile._id);
            ...
            if (!userRequestSnap.exists()) {
                await setDoc(userRequestRef, {
                    healthWorkerId: healthWorkerProfile._id,
                    userEmail: healthWorkerProfile.email,
                    userName: fullName,
